...
def chiplet_elaborator(design): 
  [tech, lanes_per_server, IO_bandwidth, silicon_per_lane, 
   chiplets_per_lane, mac_area_ratio, chip_id] = design
  ...
  if chip_id >= 0:
   ...
   # Calculate power
    ...
    if asic_spec['watts_per_mm2'] > 1.0:
        print('TOO HOT!! in tech of', tech, tops,'TOPS, ', mem_size, 'MB', ', die area is', die_area, 'has power of ', asic_spec['watts_per_asic'], 
              '(',                        (asic_spec['watts_per_asic']/die_area), 'W/mm2)')
        return
    ...
    if max_die_power <= asic_spec['watts_per_asic']:
      if keep_large_power:
        asic_spec['asic_hot'] = 1.0
      else:
        print('TOO HOT!! in tech of', tech, tops,'TOPS, ', mem_size, 'MB', 
              ', die area is', die_area, 'has power of ', asic_spec['watts_per_asic'], 
              '(',                        (asic_spec['watts_per_asic']/die_area), 
              'W/mm2) and the max power is', max_die_power)
        return
    else:
    ...
    # HBM + large chip
    die_area = silicon_per_lane / chiplets_per_lane
    asic_spec['die_area'] = die_area
    asic_spec['io_bw'] = 100
    tops = 312
    asic_spec['tops_per_asic'] = tops
    # asic_spec['sram_per_asic'] = 87
    asic_spec['w_lgc']  = 400
    asic_spec['w_sram'] = 0.0
    asic_spec['w_io']   = 0.0
    asic_spec['watts_per_asic'] = asic_spec['w_lgc'] + asic_spec['w_sram'] + asic_spec['w_io']
    asic_spec['dram_count'] = 10 # 10*8 GB
    mem_size = 80*1024
    asic_spec['sram_per_asic'] = mem_size # it's dram actually
    hs_cost = [100.0, 0, 0, 0, 0]
    max_die_power = 400 
  elif chip_id == -2:
    # HBM + small chip
    die_area = silicon_per_lane / chiplets_per_lane
    ...
    tops = 138.0/2
    asic_spec['tops_per_asic'] = tops
    # asic_spec['sram_per_asic'] = 128.0/2
    asic_spec['w_lgc']  = 175.0/2
    asic_spec['w_sram'] = 0.0
    asic_spec['w_io']   = 0.0
    asic_spec['watts_per_asic'] = asic_spec['w_lgc'] + asic_spec['w_sram'] + asic_spec['w_io']
    asic_spec['dram_count'] = 1 # 8 GB
    mem_size = 8*1024
    asic_spec['sram_per_asic'] = mem_size # it's dram actually
    hs_cost = [100.0, 0, 0, 0, 0]
    max_die_power = 175.0/2
  ...
  srv_spec = ServerCost.evalServerCost(asic_spec, die_cost, hs_cost, chiplets_per_lane, 
                                       srv_chiplets, srv_tops, lanes_per_server,
                                       chip_id)
