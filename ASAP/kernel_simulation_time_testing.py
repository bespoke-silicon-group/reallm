...
for i, matmul_size in enumerate(all_kernel_sizes['matmul']):
    ...
    if ((i+1) % 100 == 0):
        print(f'{i+1} matmuls done in {time.time() - start} seconds')
    ...
    print(f'{lat * 1e6} us')
    f.close()
print(f'{len(all_kernel_sizes["matmul"])} matmuls done in {time.time() - start} seconds')
# %%
