...
def get_latency(sys_spec, algo_spec, mapping, ts=None, prefill=None, verbose=False):
    ...
    atten_qkv_util = min(max_mac_parall/chip_macs, 1.0)
    ...
    atten_fc_util = min(max_mac_parall/chip_macs, 1.0)
    ...
    fc1_util = min(max_mac_parall/chip_macs, 1.0)
    ...
    # so the max macs per cycle is: 
    max_mac_parall = activation_row * d
    fc2_util = min(max_mac_parall/chip_macs, 1.0)
    ...
    if layers_per_pipe_stage >=1:
      ...
      pipe_stage_delay_1 = sum(atten_delays_1) + sum(fc1_delays_1) + stage2stage_delay*4/t
      # (fc2, atten)
