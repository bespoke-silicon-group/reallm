...
def lgc_sram_area_calc(asic_spec, mac_ratio):
  side = math.sqrt(asic_spec['die_area'])
  L = 0.35
  core_side = side - L
  core_area = core_side * core_side
  core_area = core_area * 0.7
  lgc_sram_total_area = core_area - asic_spec['io_area'] - \
                        asic_spec['other_area']
  if lgc_sram_total_area < 0.0:
    print('Die size too small! Not enough area for MACs and SRAM')
    return False
  else:
    asic_spec['lgc_area'] = lgc_sram_total_area * mac_ratio
    asic_spec['sram_area'] = lgc_sram_total_area - asic_spec['lgc_area']
  return True
